...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if (!interaction.guild.me.permissions.has(Permissions.FLAGS.MANAGE_GUILD))
            return interaction.channel.send(`Error: No Permission "MANAGE_GUILD", I can't check invites.`);
        ...
        if (guildInvitesFiltered.size > 0) {
            ...
            for (let invite of guildInvitesFiltered) {
                ...
                guildInvitesEmbed.addField(`[${count}\\${guildInvitesFiltered.size}] ${invite[1].url}`,
                    ...
                    Max uses: \`${(invite[1].maxUses > 0 ? "unlimited" : invite[1].maxUses)}\`;
